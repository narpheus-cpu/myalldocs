...
자동 가져오기는 Google Drive의 서버 측 복사를 사용하므로 브라우저가 JSON을 내려받았다가 다시 올리지 않습니다. JSON 한 파일의 상한은 수동 업로드와 같은 100MB입니다. 이미 수동으로 등록된 도서가 다시 대기열에 들어가더라도 최종 단계에서 Drive 원본 ID와 원문 해시를 다시 확인하여 중복 도서를 만들지 않고 `SKIPPED` 처리합니다. 실패한 대기열은 같은 JSON을 새로 복사하지 말고 기존 항목의 **다시 처리**를 사용하세요.
