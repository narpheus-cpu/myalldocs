...
> 기존 Apps Script를 이미 배포한 사용자는 `apps-script/Code.gs`를 다시 붙여넣고 **배포 → 배포 관리 → 수정 → 새 버전 → 배포**를 한 번 수행해야 작품 정보·태그 편집, 빠른 인덱싱 내용 저장, **원문 연결 수정·원문 없음**, 선택 도서 삭제, 현재 작업 폴더 복원, 사전 단계 실패 감지, **대용량 완성 JSON의 2MB 조각 업로드**가 활성화됩니다. 기존 `/exec` 주소는 그대로 사용합니다.
