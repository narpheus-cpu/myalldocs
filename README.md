...
브라우저 업로드는 파일당 100MB까지 받으며 128KB 조각으로 비공개 관리 폴더에 전송합니다. 일시적인 Apps Script 404·429·5xx 응답은 동일 조각을 최대 3회 안전하게 재전송합니다. 큰 파일은 조각 수만큼 Apps Script 요청이 필요하므로 브라우저를 닫지 말고 업로드 완료 및 대기열 등록 메시지가 나타날 때까지 기다리세요. 등록이 끝난 뒤에는 페이지를 닫거나 새로고침해도 GitHub Actions가 백엔드에서 계속 처리합니다.
