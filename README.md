...
> 2026-09-16 변경: 사이트의 **JSON 생성기** 탭에서 로컬 TXT/EPUB를 선택해 도서 목록 업로드용 JSONL을 만들 수 있습니다. 파일 내용은 브라우저 안에서만 읽고 외부로 전송하지 않습니다. 신규 자동 등록은 `[인덱싱] → [도서 목록 JSONL 업로드]`, 이미 완성된 결과는 별도의 `[완성 인덱싱 JSON 업로드]`를 사용합니다.
...
- 인덱싱 내용 편집은 용량이 큰 IndexedDB에 먼저 즉시 보존하고 GitHub에는 백그라운드 동기화. 작은 localStorage 한도를 소모하지 않으므로 글자 크기·줄간격·필터·정렬·읽던 페이지 저장과 충돌하지 않음. GitHub 직접 저장이 불가능한 토큰이면 기존 Actions 경로로 자동 전환
...
- 분석 화면은 흰 바탕의 문서형 목록으로 표시하며, `설명` 같은 내부 JSON 항목명은 숨기고 모든 원문 구간을 `제1구간`, `제2구간`처럼 간결하게 표시
- `근거 구간 보기`를 켜면 각 분석 항목 아래에서 근거 원문 구간을 선택할 수 있으며, 선택한 원문은 Google Drive에서 그때만 읽어 팝업으로 표시하고 복사 가능
- 원문 팝업의 `출력 형식 관리`에서 복사 프롬프트를 추가·수정·삭제하고 ≡ 손잡이로 정렬 가능. 선택한 프롬프트와 원문을 `프롬프트: 원문` 형태로 한 번에 복사
- 도서 상세 화면의 `작품 정보 편집`에서 작품명·작가명·장르·문서 유형·태그를 수정. 수동 값은 `data/metadata-overrides.json`에 저장되어 자동 판정보다 우선하고 재인덱싱 후에도 유지
- 새 인덱싱 결과의 태그는 `원문 내용에서 판별한 한국어 장르 → 숫자를 제거한 소속 폴더명 → 원문 기반 주제 태그` 순으로 구성
- 새로 생성하는 전체 요약은 번호 제목과 하이픈 목록을 사용하는 한국어 마크다운 개조식 보고서 형식
...
브라우저 업로드는 파일당 100MB까지 받으며 최신 Apps Script에서는 최대 2MB 조각으로 비공개 관리 폴더에 전송합니다. 업로드 시작 때 Google 계정을 한 번 확인한 뒤 임시 업로드 세션으로 이어 보내므로, 128KB 고정 조각을 수십 번 전송할 때 발생하던 반복 404를 피합니다. 일시적인 Apps Script 404·429·5xx 응답은 같은 조각을 최대 5회 안전하게 재전송합니다. 브라우저에는 조각 진행률이 표시되며, 업로드 완료 및 대기열 등록 메시지가 나타날 때까지 페이지를 닫지 마세요. 등록이 끝난 뒤에는 페이지를 닫거나 새로고침해도 GitHub Actions가 백엔드에서 계속 처리합니다.

처음 업로드할 때는 업로드 버튼이 먼저 Google 로그인 창을 엽니다. 연결 완료 문구가 나타나면 같은 업로드 버튼을 한 번 더 눌러 파일을 고릅니다. 파일 선택 창을 닫은 뒤 뒤늦게 로그인 팝업을 띄우지 않도록 이 순서를 분리했습니다.

### JSONL 입력의 최소 형태

코딩 없이 만들려면 사이트 위쪽의 **JSON 생성기** 탭을 엽니다. 파일 또는 폴더를 고르고 `JSONL 만들기` → `JSONL 다운로드`를 누른 뒤, 내려받은 파일을 인덱싱 화면의 `도서 목록 JSONL 업로드`에 넣으면 됩니다. TXT는 UTF-8과 CP949/EUC-KR을 지원하며 EPUB는 OPF 읽기 순서대로 본문을 추출합니다.
...
#### `books-json` 폴더 자동 가져오기

완성 JSON을 매번 파일 선택 창에서 올리지 않아도 됩니다.

1. Google Drive에 `books-json` 폴더를 만들고 완성된 `.json` 파일을 넣습니다. 하위 폴더가 아닌 이 폴더 바로 아래의 JSON만 대상입니다.
2. 사이트의 **인덱싱 → 완성 인덱싱 JSON 업로드 → books-json 폴더 연결**을 누르고 그 폴더를 한 번 선택합니다. 이름이 정확히 `books-json`인 폴더가 Drive에 하나뿐이면 예약 실행이 자동으로 찾아 연결할 수도 있습니다.
3. 연결하면 약 1분 안에 첫 확인을 시작하고, 이후 무료 Apps Script 시간 트리거가 약 15분마다 폴더를 다시 확인합니다. `queue-worker.yml`의 예약 실행도 30분 간격의 대기열 처리 안전망으로 남습니다. 웹페이지를 닫아도 계속됩니다. 다시 확인하려면 **지금 확인**을 누릅니다.
4. 새 파일은 생성 시각 순서로 비공개 대기열에 들어가며 한 번에 최대 5개를 등록합니다. 남은 파일은 다음 확인에서 이어집니다. 폴더 연결·**지금 확인** 버튼은 오래 기다리지 않고 별도 예약 확인만 요청하므로 브라우저의 Apps Script 응답 시간 초과를 피합니다. GitHub의 대기열 조회도 긴 폴더 스캔을 실행하지 않습니다.
5. Drive 파일 ID와 파일 버전 지문을 `서재지도_비공개_관리/books-json-import-history.json`에 기록합니다. 같은 Drive 파일의 같은 버전과 내용이 같은 복사본은 건너뜁니다. 원본 JSON은 이동하거나 삭제하지 않습니다.

자동 가져오기는 Google Drive의 서버 측 복사를 사용하므로 브라우저가 JSON을 내려받았다가 다시 올리지 않습니다. JSON 한 파일의 상한은 수동 업로드와 같은 100MB입니다. 이미 수동으로 등록된 도서가 다시 대기열에 들어가더라도 최종 단계에서 Drive 원본 ID와 원문 해시를 다시 확인하여 중복 도서를 만들지 않고 `SKIPPED` 처리합니다.

`Code.gs`의 `CANONICAL_RECONCILIATION_VERSION`이 올라가면 기존 `books-json` 파일도 파일당 한 번씩 **과거 누락 자동 구제** 대기열에 다시 들어갑니다. 이때 기존 도서와 사용자가 고친 내용은 덮어쓰지 않고, 현재 공개 목록에 없는 원본만 등록합니다. 먼저 GitHub에 저장된 완성 인덱싱 내용과 대조하므로 이미 전부 반영된 JSON은 Drive 원본 목록을 다시 훑지 않고 즉시 건너뜁니다. 일부라도 누락된 JSON만 Drive에서 원본을 확인합니다. 한 번에 최대 5개 JSON만 재검사하며 다음 15분 실행에서 이어집니다. 각 도서의 일시적인 Drive 읽기·파싱·저장 오류는 `queue.reconciliationMaxRetries` 횟수만큼 자동 재시도하고, 그래도 실패한 항목만 사용자 확인 목록에 남깁니다. 완성 JSON 재검사는 Gemini를 호출하지 않습니다.

새 파일을 업로드하면 대기열 화면은 방금 업로드한 파일의 항목만 보여 줍니다. 이전 대기열이 `NEEDS_USER_REVIEW` 또는 `ERROR`로 끝났더라도 새 작업 앞을 막지 않도록 자동으로 검토 목록으로 분리합니다. 검토 목록의 기본 화면에서는 이미 `COMPLETE` 또는 `SKIPPED`인 행을 숨기고, 실제로 조치가 필요한 항목만 남깁니다. `ERROR`의 **다시 처리**를 누르면 해당 항목의 상태를 재설정한 뒤 다시 실행합니다.
...
Google Drive에는 사용자가 넣어 둔 원본 TXT/EPUB만 그대로 남습니다. 청크 원문 팝업도 원문을 GitHub에 복사하지 않고, 허용된 Google 계정으로 Drive 원본을 그 순간에만 읽어 브라우저 메모리에 청크를 만듭니다. 창을 닫거나 새로고침하면 원문 메모리는 사라지지만, 마지막으로 읽은 구간과 페이지 위치는 브라우저 저장소에 남습니다.
...
> 기존 Apps Script를 이미 배포한 사용자는 `apps-script/Code.gs`를 다시 붙여넣고 **배포 → 배포 관리 → 수정 → 새 버전 → 배포**를 한 번 수행해야 작품 정보·태그 편집, 빠른 인덱싱 내용 저장, **원문 연결 수정·원문 없음**, 선택 도서 삭제, 현재 작업 폴더 복원, 사전 단계 실패 감지, **대용량 완성 JSON의 2MB 조각 업로드**, **books-json 자동 가져오기**가 활성화됩니다. 기존 `/exec` 주소는 그대로 사용합니다.
...
- `queue.reconciliationMaxRetries`: 과거 완성 JSON 누락 구제 중 한 도서에서 일시 오류가 발생했을 때의 추가 자동 재시도 횟수입니다. 기본값은 `2`이며 일반 수동 업로드에는 적용하지 않습니다.
...
### `QUEUED`에서 계속 멈춰 보일 때

GitHub 실행기가 시작되면 화면 상태가 `실행 환경 준비`로 바뀝니다. 의존성 설치나 사전 테스트에서 실패하더라도 `ERROR`와 GitHub 실행 상세 링크를 다시 전송하므로 `QUEUED`가 무한히 남지 않습니다. 콜백 자체가 일시적으로 실패한 경우에는 공개 GitHub 실행 상태를 30초 간격으로 보조 확인하여 완료·실패를 화면에 반영합니다. 과거 배포의 `QUEUED`가 남아 있으면 새 `Code.gs`를 새 버전으로 재배포한 뒤 다시 시작하세요.
