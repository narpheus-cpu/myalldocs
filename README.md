...
3. 연결 직후 새 JSON을 확인하고, 이후 무료 Apps Script 시간 트리거가 약 15분마다 폴더를 다시 확인합니다. `queue-worker.yml`의 예약 실행도 30분 간격의 보조 안전망으로 남습니다. 웹페이지를 닫아도 계속됩니다. 즉시 다시 확인하려면 **지금 확인**을 누릅니다.
4. 새 파일은 생성 시각 순서로 비공개 대기열에 들어가며 한 번에 최대 10개를 등록합니다. 남은 파일은 다음 확인에서 이어집니다. 폴더 연결·**지금 확인** 버튼은 오래 기다리지 않고 백엔드 확인 작업만 요청하므로 브라우저의 Apps Script 응답 시간 초과를 피합니다.
