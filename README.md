...
- 분석 화면은 흰 바탕의 문서형 목록으로 표시하며, `설명` 같은 내부 JSON 항목명은 숨기고 모든 구간을 `첫 번째 구간`, `두 번째 구간`처럼 한국어로 표시
...
- 도서 상세 화면의 `작품 정보 편집`에서 작품명·작가명·장르·문서 유형을 수정. 수동 값은 `data/metadata-overrides.json`에 저장되어 자동 판정보다 우선하고 재인덱싱 후에도 유지
