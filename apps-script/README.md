...
설정·배포 순서는 루트 [README.md](../README.md)의 7장을 따르세요. 이 폴더의 `Code.gs`와 `appsscript.json`에는 실제 token/secret을 넣지 않습니다. 모든 민감값은 Apps Script의 Script Properties에만 저장합니다. `Code.gs`를 갱신한 뒤에는 반드시 **배포 관리 → 수정 → 새 버전 → 배포**를 눌러야 JSONL 업로드·비공개 대기열·API Key 변경·원문 연결 수정·실시간 모니터가 실제 `/exec` URL에 적용됩니다. 새 버전 배포 후 기존 15분 트리거가 `books-json`의 과거 오류·누락 파일을 버전별로 한 번씩 재대조하며, 등록 완료 원본과 동일 내용은 자동으로 건너뜁니다.
