...
3. 왼쪽 **프로젝트 설정**에서 `appsscript.json` 표시를 켜고, 편집기에 나타난 파일 내용을 이 폴더의 `appsscript.json` 내용으로 바꿉니다.
...
6. 권한 확인 화면에서 Drive 읽기, GitHub 요청, 완료 메일 발송 권한을 승인합니다.
7. 마지막 `/exec` 주소를 복사해 루트 README 7장의 두 위치에 넣습니다.

설정·배포 순서는 루트 [README.md](../README.md)의 7장을 따르세요. 이 폴더의 `Code.gs`와 `appsscript.json`에는 실제 token/secret을 넣지 않습니다. 모든 민감값은 Apps Script의 Script Properties에만 저장합니다. `Code.gs`를 갱신한 뒤에는 반드시 **배포 관리 → 수정 → 새 버전 → 배포**를 눌러야 API Key 변경과 실시간 모니터 route가 실제 `/exec` URL에 적용됩니다.
